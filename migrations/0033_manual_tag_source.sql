-- Add the x-tag extension's manual tags to directory_entries.sources so
-- the directory's source facet can filter on them (discovered_via
-- 'manual_tag' -> 'tagged'). View body otherwise identical to
-- migration 0031's (dumped via pg_get_viewdef).
...
SELECT id AS artist_id,
    ...
    ( SELECT array_remove(array_agg(DISTINCT
                CASE
                    ...
                    WHEN (ac.discovered_via = 'manual_tag'::text) THEN 'tagged'::text
                    ELSE NULL::text
