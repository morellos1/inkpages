-- New review kinds (suspected non-artists from open harvests get queued for a
-- human instead of silently skipped) and clickable display-only links.
...
alter table review_items add constraint review_items_kind_check
    check (kind in ('one_directional_attach', 'cluster_merge', 'same_handle',
                    'singleton_gate', 'other'));

-- display_only keeps its fetch-policy meaning (never crawled), but the link
-- itself is shown — artists publish these handles to be followed.
